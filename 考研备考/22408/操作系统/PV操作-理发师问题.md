...
```mermaid
graph TD
    A[模板 1: 最简骨架] --> B[模板 2: 加 waiting + if → 满员离开]
    A --> C[模板 5: 加 empty 信号量 → 满员等待]
    B --> D[模板 3: 去 P(customer) → 忙等]
    D --> E[模板 4: else 补 P(customer) → 回到休息]
```
